...
const MyWork = () => {
  ...
  const handleDeleteUser = (user) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axiosPublic.delete(`/workdata/${user._id}`).then((res) => {
  ...
  const handleDrop = async (e) => {
    ...
    if (targetBox) {
      ...
      await axiosPublic.put(`/workdata/${draggedWorkItem._id}`, {
  ...
  return (
    <>
      {" "}
      <div
        className="lg:flex lg:mt-20 mx-5 gap-10
         "
      >
        <div
          id="left"
          onDrop={handleDrop()}
          className="bg-[#176B87] px-3 w-[500px] border "
        >
          <h1 className="text-center text-[#DAFFFB] lg:text-3xl my-5">
            To Do list
          </h1>
          {workdata?.map((works) => (
            <ul key={works._id}>
              <div className="flex">
                <li
                  draggable="true"
                  className="flex  items-center justify-around  list pl-5 rounded-xl text-[#DAFFFB] w-[600px] bg-[#04364A]  h-[65px] cursor-grab lg:pr-4 my-5"
                >
                  {works.title}
                  <br />
                  Deadline:
                  ...
                  <div className="flex gap-5">
                    <MdDetails
                      ...
                      onClick={() => setSelectedCard(works)}
                    ></MdDetails>

                    <CiEdit
                      className="text-2xl cursor-pointer"
                      onClick={() => handleEditWorkItem(works)}
                    >
                      <div className="modal-box bg-[#176B87]">
                        <form>
                          <div className="form-control">
                            <label className="label">
                              <span className="label-text text-[#DAFFFB]">
                                Title
                              </span>
                            </label>
                            <input
                          ...
                          <input
                            className="btn mt-4 text-[#DAFFFB] bg-[#176B87]"
                            type="submit"
                            value="Updated Board"
                          />
                        </form>
                    ...
                    <MdDelete
                      className="text-2xl cursor-pointer"
                      onClick={() => handleDeleteUser(works)}
                    ></MdDelete>
                  </div>
                </li>
              </div>
        ...
        <div
          ...
          className="bg-[#176B87] px-3 w-[500px] border "
        >
          <h1 className="text-[#DAFFFB] text-center lg:text-3xl my-5">
            On going list
          </h1>
        </div>
      ...
      <button
        className="btn my-4 bg-[#176B87] text-[#DAFFFB] lg:ml-[730px]"
        ...
        <div className="modal-box bg-[#176B87]">
          <form onSubmit={handleSubmit(onSubmit)}>
            <div className="form-control">
              ...
              />
              {errors.title && (
                <span className="text-red-800">Title is required</span>
              )}
            </div>
            <div className="form-control">
              <label className="label">
            ...
            <div className="form-control">
              ...
              />
              {errors.deadline && (
                <span className="text-red-800">Deadline is required</span>
              )}
            </div>
            <div className="form-control">
              <label className="label">
              ...
              />
              {errors.priority && (
                <span className="text-red-800">Priority is required</span>
              )}
            </div>
            <input
              className="btn mt-4 text-[#DAFFFB] bg-[#176B87]"
      ...
      {selectedCard && (
        <div className="bg-[#176B87] px-3 my-10 max-w-[320px] mx-auto w-[320px] border mt-4">
          <h2 className="text-[#DAFFFB] text-center text-xl font-bold mb-2">
